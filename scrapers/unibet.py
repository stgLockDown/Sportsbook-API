"""
Unibet (Kambi CDN) Scraper
Uses the Kambi offering API via Unibet operator code.
Fetches ML from listView, then spread/total from event detail.
Supports: NBA, NHL, NFL, MLB, Soccer (EPL, La Liga, etc.), Tennis
"""
import httpx
import asyncio
...
# Sport paths for Kambi listView
SPORT_PATHS = {
...
def _kambi_odds_to_decimal(odds: int) -> float:
    """Convert Kambi odds (1860) to decimal (1.86)."""
...
def _kambi_line_to_float(line: int) -> float:
    """Convert Kambi line (224000) to float (224.0)."""
    return round(line / 1000, 2)


def _parse_offer_type(offer: dict) -> Tuple[MarketType, str]:
    """Determine market type and name from a Kambi betOffer."""
...
def _parse_outcomes(outcomes: list, home: str, away: str) -> List[Outcome]:
    """Parse Kambi outcomes into our Outcome model."""
    result = []
    for oc in outcomes:
        label = oc.get("label", "")
        ...
        if oc_type == "OT_ONE" or label == home:
            ...
            name = "Under"
        elif oc_type == "OT_CROSS" or "Draw" in label or "X" == label:
            name = "Draw"
        else:
            name = label
...
async def _fetch_events_list(client: httpx.AsyncClient, sport_path: str) -> List[dict]:
    """Fetch events from listView endpoint."""
    try:
        r = await client.get(
...
async def _fetch_event_detail(client: httpx.AsyncClient, event_id: int) -> List[dict]:
    """Fetch full bet offers for a specific event."""
    try:
        r = await client.get(
...
async def fetch_sport(sport: str) -> List[SportsbookSnapshot]:
    """Fetch odds from Unibet/Kambi for a given sport key."""
    ...
    async with httpx.AsyncClient(
        ...
        # Step 2: Fetch event details concurrently (for spread/total)
        event_ids = []
        event_data_map = {}
        ...
        # Fetch details for up to 20 events concurrently
        detail_tasks = []
        for eid in event_ids[:20]:
            detail_tasks.append(_fetch_event_detail(client, eid))

        detail_results = await asyncio.gather(*detail_tasks, return_exceptions=True)

        # Map event ID to offers
        ...
        for eid in event_ids[:20]:
            ...
            # Build markets from detail offers
            ...
            for offer in all_offers:
                ...
                # Skip duplicates and non-standard markets
                if market_type == MarketType.OTHER:
                    continue

                # Only keep first of each type (main line)
...
if __name__ == "__main__":
    async def test():
        for sport in ["basketball_nba", "ice_hockey_nhl", "soccer_epl", "football_nfl", "baseball_mlb"]:
